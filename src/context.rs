...
///
/// `T` and `P` are type parameters required by the struct. `T` should identify operations
/// issued by the thread (an opcode of sorts). `P` should contain arguments/parameters
    ...
    P: Sized + Copy + Default,
{
    ...
    /// Returns true if the thread this corresponds to is a combiner.
    pub fn is_combiner(&self) -> bool {
        self.combinerf
    }

    /// Enqueues an operation identified by an opcode and its parameters
    /// onto this context's batch of pending operations.
    ///
    /// Returns true if the operation was successfully enqueued. False otherwise.
    pub fn enqueue(&mut self, opcode: T, params: P) -> bool {
        ...
        self.batch[self.nops] = (opcode, params);
        self.nops += 1;
...
mod test {
    ...
    fn test_context_enqueue() {
        let mut c = Context::<u64, u64>::default();
        assert!(c.enqueue(11, 121));
        assert_eq!(c.nops, 1);
        assert_eq!(c.batch[0], (11, 121));
    }
